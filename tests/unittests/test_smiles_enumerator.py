...
class TestSmilesEnumerator(TestCase):

    def test_smiles_enumerator(self):
        smiles = 'CC(=O)O'
        smiles_list = ['CC(=O)O', 'CCCC(=O)O', 'CCC(=O)OCC']

        enumerator1 = SmilesEnumerator(smiles, remove_duplicates=False, n_jobs=1, verbose=0)
        self.assertEqual(len(enumerator1.enumerate(n_max=1)), 1)
        self.assertEqual(len(enumerator1.enumerate(n_max=10)), 10)

        enumerator2 = SmilesEnumerator(smiles_list, remove_duplicates=False, n_jobs=-1, verbose=1)
    ...
    def test_smiles_enumerator_remove_duplicates(self):
        ...
        enumerator2 = SmilesEnumerator(smiles_list, remove_duplicates=True, n_jobs=-1, verbose=1)
        results = enumerator2.enumerate(n_max=20)
        self.assertEqual(len(results), len(smiles_list))

    def test_smiles_enumerator_with_seed(self):
        smiles_list = ['CC(=O)O', 'CCCC(=O)O)', 'CCC(=O)OCC']
        enumerator1 = SmilesEnumerator(smiles_list, remove_duplicates=False, seed=123, n_jobs=-1, verbose=0)
        enumerator2 = SmilesEnumerator(smiles_list, remove_duplicates=False, seed=123, n_jobs=-1, verbose=0)
