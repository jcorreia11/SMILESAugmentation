...
class ReactionSmilesRandomizer:
    ...
    def enumerate(self, n_max: int = 1) -> List[Union[str, List[str]]]:
        """
...
class ReactionSmilesPermutator:
    ...
    def __init__(self,
        ...
        ----------
        reaction_smiles: Union[str, List[str]]
            Reaction SMILES string or list of reaction SMILES strings to permutate.
        remove_duplicates: bool
            Whether to remove duplicates from the enumerated reaction SMILES.
        seed: int
            Random seed for reproducibility.
    ...
    def _enumerate_one(reaction_smiles,
                       ...
                       verbose: int = 0) -> List[str]:
        """
        Enumerate new reaction SMILES string from the given `reaction_smiles` string
        using permutations.
        ...
        permutated_reactants = list(itertools.permutations(reactants.split(".")))
        permutated_reactants = ['.'.join(reagent) for reagent in permutated_reactants]
        permutated_products = list(itertools.permutations(products.split(".")))
        permutated_products = ['.'.join(reagent) for reagent in permutated_products]

        results = list(itertools.product(permutated_reactants, permutated_products))
        if remove_duplicates:
            results = list(set(results))
        if len(results) > n_max:
            results = random.sample(results, n_max)
        # if reaction smiles has reactants>reagents>products
        if len(reaction_smiles.split('>>')) == 1:
            reagents = reaction_smiles.split(">")[1]
            return [f"{r_i[0]}>{reagents}>{r_i[1]}" for r_i in results]
